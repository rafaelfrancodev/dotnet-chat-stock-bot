...
**Live on Coolify** (VPS behind Cloudflare): <https://chat-stock-app.joya.services>, with <https://chat-stock-app.joya.services/health> (sql-server + rabbitmq + bus) and <https://chat-stock-bot.joya.services/health> (rabbitmq + quote provider + bus). During a redeploy the full `/health` legitimately answers **503** while `/health/live` stays 200 — that is the distinction, and why the compose healthcheck probes liveness. Coolify config: Build Pack **Docker Compose**, Base Directory `/`, Compose Location `/docker-compose.yml`. A deploy is `docker compose up --build` on the VPS — `chat-web` migrates and seeds at startup, so nothing is run by hand. Env vars live in the Coolify UI (`MSSQL_SA_PASSWORD`, `RABBITMQ_USER`, `RABBITMQ_PASSWORD` required; `FINNHUB_API_KEY`, `STOCKS_PROVIDER`, `REQUIRE_SECURE_COOKIE` optional).
